...
def render_dashboard(database, registry, controller=None) -> str:
    ...
    collect = '' if controller is None else '''<section class="card collect" style="margin-top:14px"><h2>Collection disabled</h2><p class=muted>This Phase 0 dashboard is read-only. No authenticated mutation profile exists; use the approved CLI workflow outside the dashboard.</p></section>'''
...
def serve(host: str="127.0.0.1", port: int=8300, controller=None) -> ThreadingHTTPServer:
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self):
            if urlparse(self.path).path == "/api/local-collection/status" and controller:
